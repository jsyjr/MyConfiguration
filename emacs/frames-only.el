;;; frames-only.el --- Use frames instead of Emacs windows -*- lexical-binding: t; -*-
...
;; URL: https://github.com/davidshepherd7/frames-only
...
(defcustom frames-only-kill-frame-when-buffer-killed-buffer-list
...
(defcustom frames-only-use-windows-for-completion t
...
(defcustom frames-only-use-window-functions
...
(defcustom frames-only-configuration-variables
  (list
   ...
   (list 'flycheck-display-errors-function #'frames-only-flycheck-display-errors))
...
mode please open an issue at https://github.com/davidshepherd7/frames-only/issues
to let me know."
  :type '(repeat (list symbol sexp))
  :group 'frames-only)

(defcustom frames-only-reopen-frames-from-hidden-x11-virtual-desktops
...
(defun frames-only-revertable-set (var-vals)
...
we even set variables that are not currently bound, but we unbind them again on revert."
  ...
  (let ((var-val-initials (-map (lambda (s) (append s
                                               (list (when (boundp (car s))
                                                     ...
                                                     (boundp (car s)))))
                                var-vals)))
    (frames-only--revertable-set-helper var-val-initials)))

(defun frames-only--revertable-set-helper (var-value-initials)
  "Internal function."
  (let ((revert-done nil)
        (revert-var-fn
         (lambda (s)
...
(defvar frames-only--revert-fn #'ignore
...
(defun frames-only-advice-use-windows (fun &rest args)
  "Create new emacs windows instead of frames within this FUN."
  (let ((pop-up-frames nil))
    (apply fun args)))


(defun frames-only-abort-recursive-edit ()
...
(defun frames-only-kill-frame-if-current-buffer-matches ()
...
packages spamming frames."
  (when (and (one-window-p)
             (member (buffer-name) frames-only-kill-frame-when-buffer-killed-buffer-list))
    (delete-frame)))


(defun frames-only-advice-use-windows-for-completion (orig-fun &rest args)
  "Advise a completion function to not use frames."
  (let ((pop-up-frames (not frames-only-use-windows-for-completion))
        (split-width-threshold 9999))
    (apply orig-fun args)))


(defun frames-only-advice-delete-frame-on-bury (orig-fun &rest args)
  "Delete the frame when burying certain buffers.

Only if there are no other windows in the frame, and if the buffer
is in frames-only-kill-frame-when-buffer-killed-buffer-list."
...
(defun frames-only-bury-completions ()
  (when (get-buffer "*Completions*")
    (bury-buffer "*Completions*")))

(defun frames-only-flycheck-display-errors (errors)
...
(defun frames-only--call-process (process &rest args)
...
(defun frames-only--x-current-desktop ()
  "Get the number of the X11 desktop which is visible"
  (--> (frames-only--call-process "wmctrl" "-d")
       (s-split "\n" it)
       (--map (split-string it "\\s-+") it)
       (--first (equal (nth 1 it) "*") it)
       (car it)))

(defun frames-only--x-visible-window-names ()
  "Get a list of X11 windows which are on the visible desktop."
  (let ((current-desktop (frames-only--x-current-desktop)))
    (--> (frames-only--call-process "wmctrl" "-l")
         (s-split "\n" it)
         (--map (s-split-up-to "\\s-+" it 3) it)
         (--filter (equal (nth 1 it) current-desktop) it)
         (--map (nth 3 it) it))))

(defun frames-only--x-buffer-window-visible (buffer-name)
  "Check buffer is currently displayed on a visible X11 virtual desktop."
  (--some (equal it buffer-name)
          (frames-only--x-visible-window-names)))

(defun frames-only--should-force-new-frame (buffer-name _)
  "See `frames-only-reopen-frames-from-hidden-x11-virtual-desktops'."
  (condition-case err
      (and frames-only-reopen-frames-from-hidden-x11-virtual-desktops
           (not (frames-only--x-buffer-window-visible buffer-name)))
    (error
     (progn
       (message "frames-only-mode failed to get a list of visible buffers from X11 with the error: '%s'
You may want to try installing `wmctrl', or disable this feature by setting `frames-only-reopen-frames-from-hidden-x11-virtual-desktops' to nil"
                err)
       nil))))
...
(defvar frames-only-mode-map
  (let ((map (make-sparse-keymap)))
    (define-key map (kbd "C-]") #'frames-only-abort-recursive-edit)
    map)
...
The following variables can be set or customized to configure behavior:
  ...
  :group      frames-only
  :global     t
  :variable   frames-only-mode
  :init-value nil
  :keymap     frames-only-mode-map

  ;; Apply most of the configuration changes
  (if frames-only-mode
      (setq frames-only--revert-fn (frames-only-revertable-set frames-only-configuration-variables))
    (funcall frames-only--revert-fn))

  ;; Disable in some functions as specified by customisation
  (if frames-only-mode
      (mapc (lambda (fun) (advice-add fun :around #'frames-only-advice-use-windows)) frames-only-use-window-functions)
    (mapc (lambda (fun) (advice-remove fun #'frames-only-advice-use-windows)) frames-only-use-window-functions))

  ;; Hacks to make other things play nice by killing the frame when certain
  ;; buffers are closed.
  (if frames-only-mode
      (progn
        (add-hook 'kill-buffer-hook #'frames-only-kill-frame-if-current-buffer-matches)
        (advice-add #'bury-buffer :around #'frames-only-advice-delete-frame-on-bury))
    (remove-hook 'kill-buffer-hook #'frames-only-kill-frame-if-current-buffer-matches)
    (advice-remove #'bury-buffer #'frames-only-advice-delete-frame-on-bury))

  ;; Advise completion popup functions to use windows instead of frames if
  ;; the custom setting is true.
  (if frames-only-mode
      (progn
        (advice-add #'minibuffer-completion-help :around #'frames-only-advice-use-windows-for-completion)
        (advice-add 'ido-completion-help :around #'frames-only-advice-use-windows-for-completion))
    (advice-remove #'minibuffer-completion-help #'frames-only-advice-use-windows-for-completion)
    (advice-remove 'ido-completion-help #'frames-only-advice-use-windows-for-completion))

  ;; Make sure completions buffer is buried after we are done with the minibuffer
  (if frames-only-mode
      (add-hook 'minibuffer-exit-hook #'frames-only-bury-completions)
    (remove-hook 'minibuffer-exit-hook #'frames-only-bury-completions))
  ...
  (if frames-only-mode
      (add-to-list 'display-buffer-alist
                   (cons #'frames-only--should-force-new-frame (cons #'display-buffer-pop-up-frame nil)))
    (setq display-buffer-alist
          (--remove (equal (car it) #'frames-only--should-force-new-frame)
