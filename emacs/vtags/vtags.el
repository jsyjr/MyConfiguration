...
where they were found."
  ...
  (if (ring-empty-p vtags-find-marker-ring)
      (user-error "No previous locations for vtags-find invocation"))
...
The current point and the destination of this search are added to the placeholder list."
  ...
  (let (
    ...
    (if (= 0 match-count)
        (user-error "No tags %s %s" matching pattern)
...
Return count of matching tags."
  ...
  (save-excursion
    ...
    (let ((file nil)
          ...
          (and (funcall (car order) pattern)
               ...
               (progn
                 ...
                 (let* ((tag-line (save-excursion
                   ...
                   (with-current-buffer output-buf
                     (when (= 1 match-count) (insert (concat "Found in " (tagfileinfo-file tfi) ":\n")))
                     (insert output-line)))
                 ))
      ...
      (unless (= 0 match-count) (with-current-buffer output-buf (insert "\n")))
      match-count)))
    ...
    (let* ((dir (file-name-directory (tagfileinfo-file tfi)))
           (match-count (vtags-look pattern tfi
                                    (lambda (line count)
                                      ...
                                      (with-current-buffer output-buf
                                        (when (= 1 count) (insert (concat "Found in " (tagfileinfo-file tfi) ":\n")))
                                        (insert line))))))
      (unless (= 0 match-count) (set-buffer output-buf) (insert "\n"))
      match-count)))
...
(defun gtags-find-tag-in-order (
  ...
  (let (
    ...
    (when (not (= 0 status))
      (user-error (buffer-substring (point-min)(1- (point-max)))))
    (setq tag-line-list
          (split-string
           output
           "\n" t ))
    ...
    (dolist  (tag-line tag-line-list (not (= (length tag-line-list) 0)))
      ...
      (setq match-count (1+ match-count))
      (when (= 1 match-count) (insert (concat "Found by GTAGS:\n")))
  ...
  (let* ((buffer-search-extensions (if (featurep 'jka-compr)
    ...
    (if (not the-buffer)
        (if (featurep 'jka-compr)
            (user-error "File %s (with or without extensions %s) not found" file tags-compression-info-list)
...
(defsubst vtags-insert-into-buffer (the-string the-buffer property-func)
        ...
        ((eq (car tag-info) t)
             ...
             (let ((startpos (cdr (cdr tag-info)))
               ...
               (cond (line (goto-line line))
                     ...
                     (t (user-error "vtags.el BUG: bogus direct file tag")))))
...
;                                   "\\(^\\|\^m\\)" "^")
             ...
             (or found
                 ...
                 (user-error "Rerun vtags: `%s' not found in %s"
        ...
        (let ((firstline t)
          ...
          (while (not (or (eobp) (looking-at "\f")))
            ...
            (with-current-buffer out-buf
              (when firstline (insert (concat "Found tag in " (tagfileinfo-file tfi) ":\n"))
                    (setq firstline nil))
              (insert tag)
              (when (= (aref tag 0) ?\() (insert " ...)"))
              (insert "\n"))
            (forward-line 1))
          t)))))

(defun vtags-list-tags-internal (tfi file out-buf)
  (when (not (tagfileinfo-etags-p tfi))
    ...
    (let* ((dir (file-name-directory (tagfileinfo-file tfi)))
      ...
      (dolist (tag-line tag-line-list (not (= (length tag-line-list) 0)))
        (when firstline (insert (concat "Found in " (tagfileinfo-file tfi) ":\n"))
              (setq firstline nil))
        (setq entry (vtags-parse-line tag-line))
      ...
      (setq gotany
            (or
             (if (tagfileinfo-etags-p (car tables))
               ...
               (vtags-list-tags-internal (car tables) file out-buf))
             gotany))
      (setq tables (cdr tables)))
    (or gotany
...
Returns the number of matched lines."
  ...
  (if (equal "0" (tagfileinfo-sorted tfi))
      (user-error "vtags-look is being called on unsorted file %s" (tagfileinfo-file tfi))
        ...
        (when tab
            ...
            (if (or (= (aref line p) '?') (= (aref line p) ?/ ))
                ...
                (let ((start (1+ p))
                  ...
                  (if (not p)
                      (user-error "invalid pattern delimiter in %s" line)
              ...
              (if (string-match "\\([0-9]+\\)" line p)
                ...
                (user-error "invalid pattern %s" line)
                )))
    ...
    (cond
     ((markerp val)
      (or (marker-buffer val)
          (user-error "That placeholder's buffer no longer exists"))
      (switch-to-buffer (marker-buffer val))
      (goto-char val))
     ((and (consp val) (eq (car val) 'file))
      (find-file (cdr val)))
     (t
      (user-error "Placeholder doesn't contain a buffer position")))))
