...
;; Author: Roland McGrath <roland@gnu.org>, ;; Author: Edward Bishop
;; Maintainer: FSF
;; Keywords: tools
...
(defun find-dominating-tags-file ()
  "recursively searches each parent directory for a file named `tags' and returns the
path to that file or nil if a tags file is not found. Returns nil if the buffer is
not visiting a file"
  (labels
      ((find-tags-file-r (dir)
         (let* ((possible-tags-file (concat dir "tags")))
           (cond
             ((file-exists-p possible-tags-file) (throw 'found-it possible-tags-file))
             ((string= dir "/") (error "no tags file found"))
             (t (find-tags-file-r (file-name-directory (directory-file-name dir))))))))

    (let ((dir1 (if (buffer-file-name)
                     (file-name-directory buffer-file-name)
                   default-directory)))
      (if dir1
          (catch 'found-it
            (find-tags-file-r dir1))
        (error "buffer has no associated directory")))))
...
`find-tag-other-window', and `find-tag-regexp'."
  (setq tags-table-computed-list (tags-compute-list (list (find-dominating-tags-file))))
...
Return count of matching tags."
  ...
  (save-excursion
    ...
    (let ((file nil)
      ...
      (unless (= 0 match-count) (save-excursion (set-buffer output-buf) (insert "\n")))
      match-count)))
...
(defun tag-find-file-of-tag (file &optional other-window-p) ; Doc string?
